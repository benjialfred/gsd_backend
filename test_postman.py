...
def run_postman_tests():
    ...
    print("SUCCESS: GET /api/users/ (sans token) -> 401 Unauthorized (Sécurité activée)")
    ...
    print("SUCCESS: POST /api/auth/token/ -> 200 OK (Token JWT reçu avec succès)")
    ...
    print("SUCCESS: GET /api/users/ (Token valide) -> 200 OK")

    # 6. Test Academy
    response = client.get('/api/academy/courses/')
    assert response.status_code == 200
    print("SUCCESS: GET /api/academy/courses/ -> 200 OK (Académie accessible)")

    # 7. Test Matchmaking
    response = client.get('/api/matchmaking/matches/')
    assert response.status_code == 200
    print("SUCCESS: GET /api/matchmaking/matches/ -> 200 OK (Matching accessible)")

    # 8. Test Coaching
    response = client.get('/api/coaching/profiles/')
    assert response.status_code == 200
    print("SUCCESS: GET /api/coaching/profiles/ -> 200 OK (Coaching accessible)")

    # 9. Test Marketplace (Core)
    response = client.get('/api/core/resources/')
    assert response.status_code == 200
    print("SUCCESS: GET /api/core/resources/ -> 200 OK (Marketplace/Podcasts accessible)")

    print("\nFIN: Tous les endpoints REST API répondent parfaitement sous autorisation JWT. L'API est robuste.")
